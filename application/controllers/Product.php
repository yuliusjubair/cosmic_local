...
class Product extends CI_Controller {
	...
	public function index() {
	    ...
	    $data['subtitle']="Manage Product Service";
	...
	public function ajax_add() {
		...
		public function ajax_add_sertifikasi() {
	    	date_default_timezone_set('Asia/Jakarta');
	    	$this->_validate_sertifikasi();
	    	$data = array();
	    	$data['error_string'] = array();
	    	$data['inputerror'] = array();
	    	$data['status'] = TRUE;

	        $id_product = $this->input->post('modal_id_product_sert');
	        $kd_perusahaan = $this->input->post('kd_perusahaan_modal_sert');
	        $nama_perusahaan = $this->input->post('modal_nama_perusahaan_sert');
	        $nama_pj = $this->input->post('modal_nama_pj_sert');
	        $no_telp_pj = $this->input->post('modal_no_telp_pj_sert');
	        $email_pj = $this->input->post('modal_email_pj_sert');
	        //$perimeter_list = $this->input->post('modal_perimeter_list');

	        $user_id = $this->ion_auth->user()->row()->id;

	        $data = array(
	        	 "tbps_mc_id"=>$kd_perusahaan,
	        	 "tbps_nama_pj"=>$nama_pj,
	        	 "tbps_no_tlp_pj"=>$no_telp_pj,
	        	 "tbps_email_pj"=>$email_pj,
	        	 "tbps_user_insert"=>$user_id,
	        	 //"tbps_mlp_id"=>$id_product,
	        	 //"tbps_perimeter"=>$perimeter_list,
	        );
	        // echo"<pre>";print_r($data);echo"</pre>";die;
	        $this->db->insert('table_pengajuan_sertifikasi', $data);
	...
	private function _validate() {
		...
		private function _validate_sertifikasi() {
		    $data = array();
		    $data['error_string'] = array();
		    $data['inputerror'] = array();
		    $data['status'] = TRUE;

	        if($this->input->post('modal_nama_pj_sert') == '') {
	            $data['inputerror'][] = 'modal_nama_pj_sert';
	            $data['error_string'][] = 'Nama Penanggung Jawab is required';
	            $data['status'] = FALSE;
	        }

	        if($this->input->post('modal_no_telp_pj_sert') == '') {
	            $data['inputerror'][] = 'modal_no_telp_pj_sert';
	            $data['error_string'][] = 'Nomor Telepon Penanggung Jawab is required';
	            $data['status'] = FALSE;
	        }

	        if($this->input->post('modal_email_pj_sert') == '') {
	            $data['inputerror'][] = 'modal_email_pj_sert';
	            $data['error_string'][] = 'Email Penanggung Jawab is required';
	            $data['status'] = FALSE;
	        }
	...
	public function ajax_update_product() {
        ...
        $data = array(
        		...
        		"mlp_active" => $this->input->post('modal_p_status'),
